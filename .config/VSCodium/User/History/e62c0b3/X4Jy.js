...
function createForm() {
        ...
        const record = {
            ...
            photo: imgList,
            salary: function() {
                const diffInMonths = (currentDate.getFullYear() - new Date(this.regDate)).getFullYear() * 12 + currentDate.getMonth() - new Date(this.regDate).getMonth();
...
function empDetail(id) {
    ...
    let detail1 = updatedrecords.filter(record => record.id == id);
    let detail = detail1[0];
    console.log(detail);
    // photoSec.innerHTML = `
    // <img src="${detail.photo[0].image}" alt="team" style="height: 250px; width: 250px; border-radius: 10px;">
    // `;

    infoSec.innerHTML = `
        <h3><b>${addressEmp()} ${detail.name}</b></h3>
        <hr>
        <h6><b>Address:</b> ${detail.address}</h6>
        <h6><b>Email Address:</b> ${detail.email}</h6>
        <h6><b>Phone Number:</b> ${detail.phone}</h6>
        <h6><b>DOB:</b> ${detail.dob}</h6>
        <h6><b>Age:</b> ${age()}</h6>
        <h6><b>Position:</b> ${detail.post}</h6>
        <h6><b>Registered Date:</b> ${detail.regDate}</h6>
        <h6><b>Gross Salary:</b> ${detail.salary}</h6>
    ...
    function age() {
        let birthYear = new Date(detail.dob).getFullYear();
...
function validateForm(){
    ...
    const emailPattern = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{3,})+$/;
